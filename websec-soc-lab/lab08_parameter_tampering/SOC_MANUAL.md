# SOC Investigation Manual — Lab 08 — Parameter Tampering

This manual guides Security Operations Center (SOC) analysts step-by-step through investigating security events generated in **Lab 08 — Parameter Tampering**.

---

## 🔍 SOC Investigation Workflow (10-Step Checklist)

1. **Open SOC Dashboard**: Open web browser to `http://localhost:8008`.
...
5. **Analyze Attack Signature**: Check the `Attack Type` column (e.g. `PARAMETER_TAMPERING`).
...
10. **Formulate Remediation**: Recommend defensive controls (Perform strict server-side authorization checks on every request to ensure the authenticated session owns or is authorized to view the requested resource ID.).
...
| `PARAMETER_TAMPERING` | HIGH/MEDIUM | Security violation pattern detected by WAF | Monitor IP, verify block status, alert administrator |
