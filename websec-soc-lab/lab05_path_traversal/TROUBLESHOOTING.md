# Troubleshooting Guide — Lab 05 — Path Traversal

This guide addresses common issues encountered when deploying or running **Lab 05 — Path Traversal**.
...
* **Symptom**: `OSError: [Errno 98] Address already in use` on port 5005, 6005, or 8005.
* **Solution**: Run `./stop.sh` inside `ubuntu_target` and `logviewer` to kill previous background processes:
  ```bash
  cd lab05_path_traversal/ubuntu_target && ./stop.sh
...
* **Solution**: Run `./healthcheck.sh` on target to verify services are listening:
  ```bash
  cd lab05_path_traversal/ubuntu_target && ./healthcheck.sh
  ```
...
* **Solution**: Reset SQLite database and restart target services:
  ```bash
  cd lab05_path_traversal/ubuntu_target && ./reset.sh && ./start.sh
  ```
