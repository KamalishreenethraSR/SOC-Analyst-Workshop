...
if not os.path.exists(sample_file):
    with open(sample_file, "w") as f:
        f.write("Welcome to Lab 05 — Path Traversal lab environment!")

@app.route('/')
def index():
    return jsonify({
        "status": "online",
        "lab": "Lab 05 — Path Traversal",
...
@app.route('/health')
def health():
    return jsonify({"status": "OK", "lab_id": "lab05"})
...
def login():
    if request.method == 'POST':
        ...
        if "lab05" == "lab03" and ("'" in username or "OR" in username.upper()):
...
def search():
    ...
    if "lab05" == "lab03" and ("'" in q or "SELECT" in q.upper()):
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        try:
