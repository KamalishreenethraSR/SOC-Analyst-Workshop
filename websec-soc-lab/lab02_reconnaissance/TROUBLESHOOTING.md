# Troubleshooting Guide — Lab 02 — Reconnaissance

This guide addresses common issues encountered when deploying or running **Lab 02 — Reconnaissance**.
...
* **Symptom**: `OSError: [Errno 98] Address already in use` on port 5002, 6002, or 8002.
* **Solution**: Run `./stop.sh` inside `ubuntu_target` and `logviewer` to kill previous background processes:
  ```bash
  cd lab02_reconnaissance/ubuntu_target && ./stop.sh
...
* **Solution**: Run `./healthcheck.sh` on target to verify services are listening:
  ```bash
  cd lab02_reconnaissance/ubuntu_target && ./healthcheck.sh
  ```
...
* **Solution**: Reset SQLite database and restart target services:
  ```bash
  cd lab02_reconnaissance/ubuntu_target && ./reset.sh && ./start.sh
  ```
