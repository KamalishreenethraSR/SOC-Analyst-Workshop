...
if not os.path.exists(sample_file):
    with open(sample_file, "w") as f:
        f.write("Welcome to Lab 02 — Reconnaissance lab environment!")
...
@app.route('/health')
def health():
    return jsonify({"status": "OK", "lab_id": "lab02"})
...
def login():
    if request.method == 'POST':
        ...
        if "lab02" == "lab03" and ("'" in username or "OR" in username.upper()):
...
def search():
    ...
    if "lab02" == "lab03" and ("'" in q or "SELECT" in q.upper()):
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        try:
