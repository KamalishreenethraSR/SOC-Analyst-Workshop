# SOC Investigation Manual — Lab 02 — Reconnaissance

This manual guides Security Operations Center (SOC) analysts step-by-step through investigating security events generated in **Lab 02 — Reconnaissance**.

---

## 🔍 SOC Investigation Workflow (10-Step Checklist)

1. **Open SOC Dashboard**: Open web browser to `http://localhost:8002`.
...
5. **Analyze Attack Signature**: Check the `Attack Type` column (e.g. `RECONNAISSANCE`).
...
10. **Formulate Remediation**: Recommend defensive controls (Block automated scanner User-Agents at the WAF level and rate-limit rapid multi-endpoint path probing.).
...
| `RECONNAISSANCE` | HIGH/MEDIUM | Security violation pattern detected by WAF | Monitor IP, verify block status, alert administrator |
