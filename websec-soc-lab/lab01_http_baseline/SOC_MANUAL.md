# SOC Investigation Manual — Lab 01 — HTTP Baseline

This manual guides Security Operations Center (SOC) analysts step-by-step through investigating security events generated in **Lab 01 — HTTP Baseline**.

---

## 🔍 SOC Investigation Workflow (10-Step Checklist)

1. **Open SOC Dashboard**: Open web browser to `http://localhost:8001`.
...
5. **Analyze Attack Signature**: Check the `Attack Type` column (e.g. `BASELINE_TRAFFIC`).
...
10. **Formulate Remediation**: Recommend defensive controls (Log all incoming HTTP requests with timestamps, client IP addresses, methods, URLs, and User-Agents for baseline modeling.).
...
| `BASELINE_TRAFFIC` | HIGH/MEDIUM | Security violation pattern detected by WAF | Monitor IP, verify block status, alert administrator |
