# Troubleshooting Guide — Lab 01 — HTTP Baseline

This guide addresses common issues encountered when deploying or running **Lab 01 — HTTP Baseline**.
...
* **Symptom**: `OSError: [Errno 98] Address already in use` on port 5001, 6001, or 8001.
* **Solution**: Run `./stop.sh` inside `ubuntu_target` and `logviewer` to kill previous background processes:
  ```bash
  cd lab01_http_baseline/ubuntu_target && ./stop.sh
...
* **Solution**: Run `./healthcheck.sh` on target to verify services are listening:
  ```bash
  cd lab01_http_baseline/ubuntu_target && ./healthcheck.sh
  ```
...
* **Solution**: Reset SQLite database and restart target services:
  ```bash
  cd lab01_http_baseline/ubuntu_target && ./reset.sh && ./start.sh
  ```
