# SOC Investigation Manual — Lab 04 — Reflected XSS

This manual guides Security Operations Center (SOC) analysts step-by-step through investigating security events generated in **Lab 04 — Reflected XSS**.

---

## 🔍 SOC Investigation Workflow (10-Step Checklist)

1. **Open SOC Dashboard**: Open web browser to `http://localhost:8004`.
...
5. **Analyze Attack Signature**: Check the `Attack Type` column (e.g. `XSS`).
...
10. **Formulate Remediation**: Recommend defensive controls (Apply contextual HTML Entity Encoding (e.g. html.escape) to all untrusted input echoed into responses.).
...
| `XSS` | HIGH/MEDIUM | Security violation pattern detected by WAF | Monitor IP, verify block status, alert administrator |
