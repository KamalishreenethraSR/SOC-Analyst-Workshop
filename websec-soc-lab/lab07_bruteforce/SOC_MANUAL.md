# SOC Investigation Manual — Lab 07 — Brute Force

This manual guides Security Operations Center (SOC) analysts step-by-step through investigating security events generated in **Lab 07 — Brute Force**.

---

## 🔍 SOC Investigation Workflow (10-Step Checklist)

1. **Open SOC Dashboard**: Open web browser to `http://localhost:8007`.
...
5. **Analyze Attack Signature**: Check the `Attack Type` column (e.g. `BRUTE_FORCE`).
...
10. **Formulate Remediation**: Recommend defensive controls (Implement account lockout policies, IP-based rate limiting, CAPTCHA mechanisms, and Multi-Factor Authentication (MFA).).
...
| `BRUTE_FORCE` | HIGH/MEDIUM | Security violation pattern detected by WAF | Monitor IP, verify block status, alert administrator |
