# Troubleshooting Guide — Lab 07 — Brute Force

This guide addresses common issues encountered when deploying or running **Lab 07 — Brute Force**.
...
* **Symptom**: `OSError: [Errno 98] Address already in use` on port 5007, 6007, or 8007.
* **Solution**: Run `./stop.sh` inside `ubuntu_target` and `logviewer` to kill previous background processes:
  ```bash
  cd lab07_bruteforce/ubuntu_target && ./stop.sh
...
* **Solution**: Run `./healthcheck.sh` on target to verify services are listening:
  ```bash
  cd lab07_bruteforce/ubuntu_target && ./healthcheck.sh
  ```
...
* **Solution**: Reset SQLite database and restart target services:
  ```bash
  cd lab07_bruteforce/ubuntu_target && ./reset.sh && ./start.sh
  ```
