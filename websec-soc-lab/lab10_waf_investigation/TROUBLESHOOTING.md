# Troubleshooting Guide — Lab 10 — WAF Investigation

This guide addresses common issues encountered when deploying or running **Lab 10 — WAF Investigation**.
...
* **Symptom**: `OSError: [Errno 98] Address already in use` on port 5010, 6010, or 8010.
* **Solution**: Run `./stop.sh` inside `ubuntu_target` and `logviewer` to kill previous background processes:
  ```bash
  cd lab10_waf_investigation/ubuntu_target && ./stop.sh
...
* **Solution**: Run `./healthcheck.sh` on target to verify services are listening:
  ```bash
  cd lab10_waf_investigation/ubuntu_target && ./healthcheck.sh
  ```
...
* **Solution**: Reset SQLite database and restart target services:
  ```bash
  cd lab10_waf_investigation/ubuntu_target && ./reset.sh && ./start.sh
  ```
