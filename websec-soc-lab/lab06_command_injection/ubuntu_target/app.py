...
if not os.path.exists(sample_file):
    with open(sample_file, "w") as f:
        f.write("Welcome to Lab 06 — Command Injection lab environment!")

@app.route('/')
def index():
    return jsonify({
        "status": "online",
        "lab": "Lab 06 — Command Injection",
...
@app.route('/health')
def health():
    return jsonify({"status": "OK", "lab_id": "lab06"})
...
def login():
    if request.method == 'POST':
        ...
        if "lab06" == "lab03" and ("'" in username or "OR" in username.upper()):
...
def search():
    ...
    if "lab06" == "lab03" and ("'" in q or "SELECT" in q.upper()):
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        try:
