# SOC Investigation Manual — Lab 03 — SQL Injection

This manual guides Security Operations Center (SOC) analysts step-by-step through investigating security events generated in **Lab 03 — SQL Injection**.

---

## 🔍 SOC Investigation Workflow (10-Step Checklist)

1. **Open SOC Dashboard**: Open web browser to `http://localhost:8003`.
...
5. **Analyze Attack Signature**: Check the `Attack Type` column (e.g. `SQL_INJECTION`).
...
10. **Formulate Remediation**: Recommend defensive controls (Always use Parameterized Queries (Prepared Statements) in database code instead of string concatenation.).
...
| `SQL_INJECTION` | HIGH/MEDIUM | Security violation pattern detected by WAF | Monitor IP, verify block status, alert administrator |
