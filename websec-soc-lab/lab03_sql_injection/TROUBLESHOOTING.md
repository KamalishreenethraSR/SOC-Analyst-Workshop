# Troubleshooting Guide — Lab 03 — SQL Injection

This guide addresses common issues encountered when deploying or running **Lab 03 — SQL Injection**.
...
* **Symptom**: `OSError: [Errno 98] Address already in use` on port 5003, 6003, or 8003.
* **Solution**: Run `./stop.sh` inside `ubuntu_target` and `logviewer` to kill previous background processes:
  ```bash
  cd lab03_sql_injection/ubuntu_target && ./stop.sh
...
* **Solution**: Run `./healthcheck.sh` on target to verify services are listening:
  ```bash
  cd lab03_sql_injection/ubuntu_target && ./healthcheck.sh
  ```
...
* **Solution**: Reset SQLite database and restart target services:
  ```bash
  cd lab03_sql_injection/ubuntu_target && ./reset.sh && ./start.sh
  ```
